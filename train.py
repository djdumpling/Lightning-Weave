...
def train(args):
    ...
    # Keep the scheduler horizon (`num_rollout`) independent from an optional
    # process-local boundary used by segmented refresh experiments.
    rollout_end_step = args.num_rollout if args.rollout_end_step is None else args.rollout_end_step
    if not args.start_rollout_id <= rollout_end_step <= args.num_rollout:
        raise ValueError(
            "Expected start_rollout_id <= rollout_end_step <= num_rollout, got "
            f"{args.start_rollout_id} <= {rollout_end_step} <= {args.num_rollout}"
        )

    # train loop.
    # note that for async training, one can change the position of the sync operation(ray.get).
    for rollout_id in range(args.start_rollout_id, rollout_end_step):
