    ...
    def get_samples(self, num_samples):
        ...
        for prompt_sample in prompt_samples:
            ...
            for _ in range(self.args.n_samples_per_prompt):
                # A sealed Offline Direct-OPD row can contain hundreds of
                # thousands of cached candidate values.  The run-size gate
                # guarantees a one-pass dataset, and hydration only mutates
                # top-level Sample fields, so share the immutable nested row
                # payload while keeping an independent Sample object.
                sample = (
                    copy.copy(prompt_sample)
                    if getattr(self.args, "offline_direct_opd", False)
                    else copy.deepcopy(prompt_sample)
                )
