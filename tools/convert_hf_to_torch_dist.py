...
from slime.backends.megatron_utils.hf_to_megatron import (
    load_hf_weights,
    supports_hf_weight_loading,
)
...
def add_convertion_args(parser):
    ...
    if "--use-gated-attention" not in parser._option_string_actions:
        parser.add_argument(
            "--use-gated-attention", action="store_true", default=False
        )
    try:
        parser.add_argument("--padded-vocab-size", type=int, default=None)
    except Exception:
        pass
    parser.add_argument("--qwen-yarn-factor", type=float, default=None)
    parser.add_argument(
...
def main():
    ...
    if supports_hf_weight_loading(hf_model_path):
        # Qwen3.5 needs the architecture-specific gated-Q merge from upstream
        # Slime.  MBridge in the immutable image predates the architecture.
