...
from slime.utils import tracking_utils, train_dump_utils
...
class MegatronTrainRayActor(TrainRayActor):
    ...
    def init(
        ...
        if (
            role == "actor"
            and self.args.offline_direct_opd_adaptive_kl
            and self.args.save is not None
            and Path(self.args.load).resolve() == Path(self.args.save).resolve()
        ):
            from slime.rollout.offline_direct_opd import load_adaptive_kl_state

            self.args.offline_direct_opd_kl_coef = load_adaptive_kl_state(self.args, loaded_rollout_id)
            if is_megatron_main_rank():
                logger.info(
                    "Restored Offline Direct-OPD adaptive KL coefficient %.8f at rollout %d",
    ...
    def _get_rollout_data(self, rollout_data_ref: Box) -> RolloutBatch:
        ...
        rollout_data["tokens"] = [
            torch.as_tensor(t, dtype=torch.long).to(device=torch.cuda.current_device())
            for t in rollout_data["tokens"]
        ]
        rollout_data["loss_masks"] = [
            torch.as_tensor(t, dtype=torch.int).to(device=torch.cuda.current_device())
        ...
        if "rollout_log_probs" in rollout_data:
            rollout_data["rollout_log_probs"] = [
                slice_log_prob_with_cp(
                    direct_opd_cpu_tensor(log_prob, dtype=torch.float32),
                    total_length,
                    response_length,
        ...
        for key, dtype in direct_opd_dtypes.items():
            if key in rollout_data:
                rollout_data[key] = [
                    slice_log_prob_with_cp(
                        direct_opd_cpu_tensor(value, dtype=dtype),
                        total_length,
                        response_length,
                    ).to(device=torch.cuda.current_device())
    ...
    def train_actor(self, rollout_id: int, rollout_data: RolloutBatch) -> None:
        ...
        if self.args.offline_direct_opd_adaptive_kl:
            from slime.rollout.offline_direct_opd import (
                get_offline_direct_opd_reward_stats,
                update_kl_loss_coef_from_reward,
            )

            adaptive_stats = forward_only(
                get_offline_direct_opd_reward_stats,
                self.args,
                self.model,
                data_iterator,
            ...
            reward_sums = adaptive_stats.get("offline_direct_opd_reward_sum")
            reward_counts = adaptive_stats.get("offline_direct_opd_reward_count")
            if not reward_sums or not reward_counts:
                raise RuntimeError("adaptive KL prepass did not produce Direct-OPD reward statistics")
            totals = torch.stack(
                [
                    torch.stack(reward_sums).sum(),
                    torch.stack(reward_counts).sum(),
                ]
            )
            dist.all_reduce(totals, group=mpu.get_data_parallel_group())
            if totals[1].item() <= 0:
                raise RuntimeError("adaptive KL prepass found no valid candidate reward elements")
            reward_mean = (totals[0] / totals[1]).item()
            coefficient_before = float(self.args.offline_direct_opd_kl_coef)
            coefficient_after = update_kl_loss_coef_from_reward(
                current_coef=coefficient_before,
                reward_mean=reward_mean,
                eps=self.args.offline_direct_opd_kl_eps,
                min_coef=self.args.offline_direct_opd_kl_min,
                max_coef=self.args.offline_direct_opd_kl_max,
            )
            self.args.offline_direct_opd_kl_coef = coefficient_after
            if is_megatron_main_rank():
                adaptive_log = {
                    "train/adaptive_kl_reward": reward_mean,
                    "train/adaptive_kl_coef_before": coefficient_before,
                    "train/adaptive_kl_coef_after": coefficient_after,
                    "train/step": rollout_id * len(num_microbatches),
                }
                logger.info("Offline Direct-OPD adaptive KL rollout %d: %s", rollout_id, adaptive_log)
                tracking_utils.log(self.args, adaptive_log, step_key="train/step")
    ...
    def save_model(self, rollout_id: int, force_sync: bool = False) -> None:
        ...
        if self.args.offline_direct_opd_adaptive_kl:
            from slime.rollout.offline_direct_opd import save_adaptive_kl_state

            if is_megatron_main_rank():
                save_adaptive_kl_state(self.args, rollout_id)
            dist.barrier(group=get_gloo_group())
