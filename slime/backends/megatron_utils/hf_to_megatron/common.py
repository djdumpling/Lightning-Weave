...
class SafetensorReader:
    """Lazy, one-shard-at-a-time reader for an indexed HF checkpoint."""
        ...
        if index_path.is_file():
            ...
            if not files:
                raise FileNotFoundError(
                    f"No safetensors checkpoint found in {self.path}"
                )
            self.weight_map = {}
            for file in files:
                with safe_open(
                    file, framework="pt", device="cpu"
                ) as tensors:
                    self.weight_map.update(
                        dict.fromkeys(tensors.keys(), file.name)
                    )
        self._files = {}
    ...
    def get_tensor(self, name: str) -> torch.Tensor:
        try:
            ...
            raise KeyError(
                f"HuggingFace checkpoint does not contain {name!r}"
            ) from exc
        if filename not in self._files:
            self._files[filename] = safe_open(
                self.path / filename, framework="pt", device="cpu"
            )
        ...
        if tensor.element_size() == 1 and scale_name in self:
            ...
            if scale_file not in self._files:
                self._files[scale_file] = safe_open(
                    self.path / scale_file,
                    framework="pt",
                    device="cpu",
                )
            scale = self._files[scale_file].get_tensor(scale_name).to(
                torch.bfloat16
            )
...
def _tensor_parallel_shard(
    ...
    *,
    ...
    if "linear_fc1.weight" in name or "linear_fc1.bias" in name:
        ...
        gate = torch.chunk(gate, parallel_size, dim=partition_dim)[
            parallel_rank
        ]
        up = torch.chunk(up, parallel_size, dim=partition_dim)[
            parallel_rank
        ]
    ...
    chunks = torch.chunk(
        tensor, parallel_size * partition_stride, dim=partition_dim
    )
    return torch.cat(
        chunks[parallel_rank::parallel_size], dim=partition_dim
    ).contiguous()


def shard_mcore_tensor(
    name: str, tensor: torch.Tensor, parameter: torch.Tensor
) -> torch.Tensor:
...
def _pad_vocab(args, name: str, tensor: torch.Tensor) -> torch.Tensor:
    if not (
        name.endswith("embedding.word_embeddings.weight")
...
def load_model_hf_weights(
    ...
    path: str | Path,
    config,
    get_hf_tensor: Callable[
        [str, SafetensorReader, object], torch.Tensor
    ],
) -> None:
    ...
    with torch.no_grad():
        for name, parameter in named_params_and_buffers(args, model):
            ...
            if (
                ...
                continue
            tensor = shard_mcore_tensor(
                name, _pad_vocab(args, name, tensor), parameter
            )
            if tensor.shape != parameter.shape:
                raise ValueError(
                    f"Shape mismatch loading {name}: HuggingFace "
                    f"{tuple(tensor.shape)}, Megatron "
