...
def get_model_provider_func(
    ...
    def model_provider(pre_process: bool = True, post_process: bool = True, vp_stage: int | None = None) -> GPTModel:
        ...
        if args.qwen_yarn_factor is not None:
            from slime_plugins.models.qwen_yarn_rotary import (
                QwenYarnRotaryEmbedding,
            )

            if (
                args.qwen_yarn_original_max_position_embeddings is None
                or args.qwen_yarn_attention_factor is None
            ):
                raise ValueError(
                    "Qwen YaRN requires factor, original max positions, "
                    "and attention factor"
                )
            old_rotary = model.rotary_pos_emb
            model.rotary_pos_emb = QwenYarnRotaryEmbedding(
                kv_channels=config.kv_channels,
                rotary_percent=args.rotary_percent,
                rotary_interleaved=config.rotary_interleaved,
                rotary_base=args.rotary_base,
                scaling_factor=args.qwen_yarn_factor,
                original_max_position_embeddings=(
                    args.qwen_yarn_original_max_position_embeddings
                ),
                attention_factor=args.qwen_yarn_attention_factor,
                use_cpu_initialization=config.use_cpu_initialization,
                cp_group=old_rotary.cp_group,
            )
