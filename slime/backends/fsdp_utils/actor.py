...
def _load_ring_flash_attn():
    """Load the optional ring-attention adapter only for real CP runs.

    Pure data-parallel FSDP (context_parallel_size=1) never calls either
    adapter. Importing the package eagerly nevertheless couples every FSDP
    run to its private Transformers API compatibility, which prevents newer
    model families from using a newer Transformers runtime.
    """
    ...
    try:
        ...
        raise RuntimeError(
            "context_parallel_size > 1 requires a ring_flash_attn build "
            "compatible with the active Transformers version"
        ) from exc
    return substitute_hf_flash_attn, update_ring_flash_attn_params
...
class FSDPTrainRayActor(TrainRayActor):
    ...
    def _setup_device_mesh(self) -> None:
        ...
        if self.cp_size > 1:
            substitute_hf_flash_attn, _ = _load_ring_flash_attn()
        ...
        for mbs_size in num_microbatches:
            ...
            packed_batches.extend(
                pack_sequences(
                    ...
                    teacher_log_probs=(
                        rollout_data["teacher_log_probs"][start:end]
                        if "teacher_log_probs" in rollout_data
                        else None
                    ),
                    candidate_ids=(
                        rollout_data["candidate_ids"][start:end]
                        if "candidate_ids" in rollout_data
                        else None
                    ),
                    behavior_topk_log_probs=(
                        rollout_data["behavior_topk_log_probs"][start:end]
                        if "behavior_topk_log_probs" in rollout_data
                        else None
                    ),
                    post_teacher_log_probs=(
                        rollout_data["post_teacher_log_probs"][start:end]
                        if "post_teacher_log_probs" in rollout_data
                        else None
                    ),
                    pre_teacher_log_probs=(
                        rollout_data["pre_teacher_log_probs"][start:end]
                        if "pre_teacher_log_probs" in rollout_data
                        else None
                    ),
                    student_ref_sampled_log_probs=(
                        rollout_data["student_ref_sampled_log_probs"][start:end]
                        if "student_ref_sampled_log_probs" in rollout_data
                        else None
                    ),
    ...
    def _train_core(self, rollout_id: int, rollout_data) -> None:
        if self.args.offline_direct_opd:
            # The FSDP Direct-OPD path consumes cached candidate distributions
            # directly.  Dummy token vectors preserve the generic packer
            # contract but are never used to form a PPO surrogate.
            ...
            if teacher_log_probs is None:
                raise KeyError("on_policy_distillation requires teacher_log_probs in rollout data")
            if len(teacher_log_probs) != len(rollout_data["response_lengths"]):
                raise ValueError(
                    "teacher_log_probs/response_lengths batch mismatch: "
                    f"{len(teacher_log_probs)} != {len(rollout_data['response_lengths'])}"
                )
            # The actual token advantages depend on the current student
            # log-probabilities and are filled after the no-grad actor forward.
            rollout_data["advantages"] = rollout_data["returns"] = [
                torch.zeros(response_length, dtype=torch.float32)
                for response_length in rollout_data["response_lengths"]
            ]
        ...
        if not self.args.offline_direct_opd:
            self._compute_log_prob("actor", packed_batches)
            if self.args.advantage_estimator == "on_policy_distillation":
                self._set_distillation_advantages(packed_batches)
            self._log_rollout_data(rollout_id, rollout_data, packed_batches)

        direct_opd_token_normalizers = None
        if self.args.offline_direct_opd:
            direct_opd_token_normalizers = self._direct_opd_token_normalizers(
                packed_batches,
                grad_accum,
            )
        ...
        with timer("actor_train"):
            ...
            for mbs_id, packed_batch in self.prof.iterate_train_actor(
                ...
                self._train_step(
                    ...
                    direct_opd_token_normalizer=(
                        direct_opd_token_normalizers[mbs_id]
                        if direct_opd_token_normalizers is not None
                        else None
                    ),
                )
    ...
    @staticmethod
    def _set_distillation_advantages(packed_batches: list[dict[str, torch.Tensor]]) -> None:
        """Set stop-gradient token advantages to log p_teacher - log p_student."""
        for packed_batch in packed_batches:
            unpacked = unpack_sequences(packed_batch)
            advantages = []
            for sequence_id, sequence in enumerate(unpacked):
                student = sequence["log_probs"].detach()
                teacher = sequence["teacher_log_probs"].to(
                    device=student.device,
                    dtype=student.dtype,
                )
                if teacher.shape != student.shape:
                    raise ValueError(
                        "Teacher/student log-probability shape mismatch for "
                        f"sequence {sequence_id}: {tuple(teacher.shape)} != {tuple(student.shape)}"
                    )
                advantages.append(teacher - student)

            packed_advantages = torch.cat(advantages, dim=0)
            packed_batch["advantages"] = packed_advantages
            packed_batch["returns"] = packed_advantages.clone()

    def _direct_opd_token_normalizers(
        self,
        packed_batches: list[dict],
        grad_accum: list[int],
    ) -> list[float]:
        """Return the global trainable-token count for each packed microbatch.

        FSDP averages gradients across data-parallel ranks.  Direct-OPD uses
        the same global token-mean reduction as the Megatron implementation,
        so every microbatch in one optimizer step shares a denominator summed
        over all DP ranks.
        """

        normalizers: list[float] = [0.0] * len(packed_batches)
        group_start = 0
        for group_end in grad_accum:
            local_tokens = sum(
                float(batch["loss_masks"].sum().item())
                for batch in packed_batches[group_start:group_end]
            )
            ...
            dist.all_reduce(global_tokens, op=dist.ReduceOp.SUM, group=self.dp_group)
            denominator = float(global_tokens.item())
            if denominator <= 0:
                raise ValueError("Offline Direct-OPD optimizer step has no trainable tokens")
            for index in range(group_start, group_end):
                normalizers[index] = denominator
            group_start = group_end
        if group_start != len(packed_batches):
            raise ValueError(
                f"grad_accum covers {group_start} packed batches, "
                f"but {len(packed_batches)} were produced"
            )
        return normalizers

    def _offline_direct_opd_loss(
        self,
        *,
        logits: torch.Tensor,
        packed_batch: dict,
        global_token_normalizer: float,
    ) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """Compute the native-HF/FSDP tilted-target Direct-OPD objective."""
        ...
        if self.cp_size != 1:
            raise ValueError("Offline Direct-OPD FSDP requires context parallel size 1")
        unpacked_batches = unpack_sequences(packed_batch)
        token_terms: dict[str, list[torch.Tensor]] = {}
        sequence_offset = 0
        temperature = float(self.args.rollout_temperature)
        for sample_index, batch in enumerate(unpacked_batches):
            missing = [
                field for field in DIRECT_OPD_SAMPLE_FIELDS if batch.get(field) is None
            ]
            if missing:
                raise KeyError(
                    "Offline Direct-OPD FSDP batch is missing fields for "
                    f"sample {sample_index}: {', '.join(missing)}"
                )
            total_length = int(batch["tokens"].numel())
            response_length = int(batch["response_lengths"])
            prompt_length = total_length - response_length
            if prompt_length <= 0:
                raise ValueError(
                    f"sample {sample_index}: prompt length must be positive, "
                    f"got total={total_length}, response={response_length}"
                )
            response_logits = logits[
                sequence_offset + prompt_length - 1 :
                sequence_offset + total_length - 1
            ]
            ...
            if candidate_ids.shape[0] != response_logits.shape[0]:
                raise ValueError(
                    f"sample {sample_index}: candidate length "
                    f"{candidate_ids.shape[0]} != logits length "
                    f"{response_logits.shape[0]}"
                )
            candidate_log_probs = dense_candidate_log_probs(
                response_logits,
                candidate_ids,
            ...
            loss_mode = self.args.offline_direct_opd_loss_mode
            if loss_mode == "tilted_target":
                terms = offline_direct_opd_tilted_target_terms(
                    candidate_log_probs,
                    batch["behavior_topk_log_probs"].to(response_logits.device),
                    batch["post_teacher_log_probs"].to(response_logits.device),
                    batch["pre_teacher_log_probs"].to(response_logits.device),
                    alpha=float(self.args.offline_direct_opd_kl_coef),
                )
            elif loss_mode == "policy_gradient":
                scaled_logits = response_logits.float() / temperature
                sampled_tokens = batch["tokens"][-response_length:].to(
                    device=response_logits.device,
                    dtype=torch.long,
                )
                sampled_log_probs = scaled_logits.gather(
                    dim=-1,
                    index=sampled_tokens.unsqueeze(-1),
                ).squeeze(-1) - scaled_logits.logsumexp(dim=-1)
                terms = offline_direct_opd_terms(
                    candidate_log_probs,
                    sampled_log_probs,
                    batch["post_teacher_log_probs"].to(response_logits.device),
                    batch["pre_teacher_log_probs"].to(response_logits.device),
                    batch["student_ref_sampled_log_probs"].to(response_logits.device),
                    batch["behavior_topk_log_probs"].to(response_logits.device),
                )
            else:
                raise ValueError(
                    f"unknown Offline Direct-OPD loss mode {loss_mode!r}"
                )
            loss_mask = batch["loss_masks"].to(response_logits.device)
            for name, values in terms.items():
                token_terms.setdefault(name, []).append((values * loss_mask).sum())
        if sequence_offset != logits.shape[0]:
            raise ValueError(
                f"packed sequence lengths cover {sequence_offset} logits, "
                f"but model returned {logits.shape[0]}"
            )

        # The generic FSDP tail multiplies by dp_size/global_batch_size.  Scale
        # token sums here so the resulting gradient is
        # dp_size * local_token_sum / global_trainable_tokens, exactly
        # compensating FSDP's DP gradient average.
        metric_scale = float(self.args.global_batch_size) / global_token_normalizer
        reduced = {
            name: torch.stack(values).sum() * metric_scale
            for name, values in token_terms.items()
        }
        if self.args.offline_direct_opd_loss_mode == "tilted_target":
            ...
            loss = reduced["direct_loss"] + float(
                self.args.offline_direct_opd_kl_coef
            ) * reduced["kl_loss"]
            log_keys = (
                "direct_loss",
                "kl_loss",
        ...
        reported.update({name: reduced[name].detach() for name in log_keys})
        reported[
            "tilt_alpha"
            if self.args.offline_direct_opd_loss_mode == "tilted_target"
            else "kl_coef"
        ] = loss.detach().new_tensor(
            float(self.args.offline_direct_opd_kl_coef)
            * float(self.args.global_batch_size)
            * sum(int(batch["loss_masks"].sum().item()) for batch in unpacked_batches)
            / global_token_normalizer
        )
    ...
    def _train_step(
        ...
        mbs_id,
        grad_accum,
        direct_opd_token_normalizer=None,
    ):
        ...
        if self.args.offline_direct_opd:
            if direct_opd_token_normalizer is None:
                raise ValueError("missing Offline Direct-OPD token normalizer")
            loss, reported = self._offline_direct_opd_loss(
                logits=logits,
                packed_batch=packed_batch,
                global_token_normalizer=direct_opd_token_normalizer,
            )
            # Continue in the shared optimizer/gradient-accumulation tail.
            loss = loss * self.dp_size / self.args.global_batch_size
            loss.backward()
            for key, value in reported.items():
                reported_accum.setdefault(key, []).append(value)
    ...
    def _finish_train_step(self, *, reported_accum, mbs_id, grad_accum):
    ...
    def update_weights(self) -> None:  # type: ignore[override]
        ...
        # Offline training (including Lightning OPD) has no rollout engines.
        # There is nothing to synchronize, and the distributed updater has not
        # established a source rank until at least one engine is connected.
    ...
    def _get_model_inputs_args(self, packed_sequence: dict) -> dict:
        ...
        if self.cp_size > 1:
            ...
            _, update_ring_flash_attn_params = _load_ring_flash_attn()
...
def apply_fsdp2(model, mesh=None, cpu_offload=False, args=None):
    ...
    raw_layer_classes = getattr(model, "_no_split_modules", None)
    if isinstance(raw_layer_classes, (set, frozenset)):
        layer_cls_to_wrap = tuple(sorted(raw_layer_classes))
    elif raw_layer_classes is not None:
        layer_cls_to_wrap = tuple(raw_layer_classes)
    else:
        layer_cls_to_wrap = ()
    if not layer_cls_to_wrap or any(
        not isinstance(name, str) or not name for name in layer_cls_to_wrap
    ):
        raise ValueError(
            "model._no_split_modules must contain at least one module class name"
        )
