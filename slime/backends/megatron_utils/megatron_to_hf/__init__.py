...
def convert_to_hf(args, model_name, name, param, quantization_config=None):
    ...
    # FP8 conversion pulls in optional SGLang/DeepGEMM components.  Keep the
    # ordinary BF16/FP32 checkpoint conversion path usable in CPU-only
    # environments where those quantization dependencies are not installed.
...
def _convert_to_hf_core(args, model_name, name, param):
    ...
    if "qwen35" in normalized_model_name:
        converted_named_tensors = convert_qwen3_5_to_hf(
            args, name, param
        )
