    ...
    def add_slime_arguments(parser):
        ...
        def add_train_arguments(parser):
            reset_arg(
                ...
                default=False,
                help=(
                    "Enable Qwen's Q+gate+K+V attention projection in "
                    "the MCore 0.14 runtime."
                ),
            )
            parser.add_argument(
                "--qwen-gdn-backend",
                ...
                default="fla",
                help=(
                    "GDN implementation backend for Qwen "
            ...
            parser.add_argument(
                ...
                type=float,
                default=None,
                help="YaRN interpolation factor for a dense Qwen checkpoint.",
            )
            parser.add_argument(
                "--qwen-yarn-original-max-position-embeddings",
                type=int,
                default=None,
                help="Original context length used to construct Qwen YaRN.",
            )
            parser.add_argument(
                "--qwen-yarn-attention-factor",
                type=float,
                default=None,
                help="Qwen/vLLM YaRN cosine/sine magnitude correction.",
            )
            parser.add_argument(
                "--train-backend",
                type=str,
        ...
        def add_data_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Exclusive cumulative rollout boundary for this process. "
                    "The scheduler horizon remains controlled by --num-rollout."
                ),
            )
            parser.add_argument(
                "--num-epoch",
        ...
        def add_algo_arguments(parser):
            ...
            reset_arg(parser, "--override-opt-param-scheduler", action="store_true", default=False)
            ...
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Train from sealed Offline Direct-OPD response/candidate/teacher-score data.",
            )
            parser.add_argument(
                "--direct-opd-top-k",
                type=int,
                default=16,
                help="Expected cached student candidate count for Offline Direct-OPD.",
            )
            parser.add_argument(
                "--offline-direct-opd-kl-coef",
                type=float,
                default=0.0,
                help="Initial-student sampled-token low-var KL coefficient for Offline Direct-OPD.",
            )
            parser.add_argument(
            ...
            parser.add_argument(
                "--offline-direct-opd-allow-student-mismatch",
                action="store_true",
                default=False,
                help=(
                    "Allow a tilted-target diagnostic run to initialize from an HF checkpoint "
                    "different from the sealed behavior student. The cached behavior distribution "
                    "remains the target base; this is intended for counterfactual occupancy studies."
                ),
            )
            parser.add_argument(
                "--offline-direct-opd-adaptive-kl",
                action="store_true",
                default=False,
                help="Update the Offline Direct-OPD KL coefficient once per whole rollout using reward sign.",
            )
            parser.add_argument(
                "--offline-direct-opd-kl-eps",
                type=float,
                default=0.01,
                help="Multiplicative sign-controller step for adaptive Offline Direct-OPD KL.",
            )
            parser.add_argument(
                "--offline-direct-opd-kl-min",
                type=float,
                default=0.5,
                help="Lower bound for the adaptive Offline Direct-OPD KL coefficient.",
            )
            parser.add_argument(
                "--offline-direct-opd-kl-max",
                type=float,
                default=2.5,
                help="Upper bound for the adaptive Offline Direct-OPD KL coefficient.",
            )
            parser.add_argument(
                "--offline-direct-opd-manifest",
                type=str,
                default=None,
                help="Sealed dataset manifest checked before Offline Direct-OPD training.",
            )
            parser.add_argument(
                "--offline-direct-opd-responses-per-prompt",
                type=int,
                default=4,
                help=(
                    "Expected number of sampled responses per source prompt recorded in the "
                    "sealed Offline Direct-OPD manifest. This is a data-provenance check and "
                    "does not change the offline training batch shape."
                ),
            )
            parser.add_argument(
                "--kl-loss-coef",
        ...
        def add_reward_model_arguments(parser):
            ...
            parser.add_argument(
                ...
                default=None,
                help=(
                    "Optional tokenizer used by a cross-template reward/teacher model. "
                    "Custom reward adapters can use this together with the preserved raw "
                    "prompt messages to render the reward model's native chat template."
                ),
            )
            parser.add_argument(
                "--custom-rm-path",
...
def slime_validate_args(args):
    ...
    if args.offline_direct_opd:
        expected_loss = "slime.rollout.offline_direct_opd.megatron_loss"
        if args.loss_type != "custom_loss" or args.custom_loss_function_path != expected_loss:
            raise ValueError(
                "--offline-direct-opd requires --loss-type custom_loss and "
                f"--custom-loss-function-path {expected_loss}"
            )
        ...
        if args.train_backend == "fsdp" and args.context_parallel_size != 1:
            raise ValueError("Offline Direct-OPD FSDP requires --context-parallel-size 1")
        if args.train_backend == "fsdp" and args.offline_direct_opd_adaptive_kl:
            raise ValueError("adaptive Offline Direct-OPD is not yet supported by the FSDP backend")
        if args.direct_opd_top_k <= 0:
            raise ValueError("--direct-opd-top-k must be positive")
        if args.offline_direct_opd_responses_per_prompt <= 0:
            raise ValueError("--offline-direct-opd-responses-per-prompt must be positive")
        if args.offline_direct_opd_kl_coef < 0:
            raise ValueError("--offline-direct-opd-kl-coef must be non-negative")
        if args.offline_direct_opd_loss_mode == "tilted_target" and args.offline_direct_opd_kl_coef <= 0:
        ...
        if args.offline_direct_opd_adaptive_kl:
            if (
                ...
                raise ValueError(
                    "adaptive Offline Direct-OPD v1 requires tensor/pipeline/context parallel size 1"
                )
            if not 0 <= args.offline_direct_opd_kl_eps < 1:
                raise ValueError("--offline-direct-opd-kl-eps must satisfy 0 <= eps < 1")
            if args.offline_direct_opd_kl_min < 0 or args.offline_direct_opd_kl_max < args.offline_direct_opd_kl_min:
                raise ValueError("invalid adaptive Offline Direct-OPD KL bounds")
            if not args.offline_direct_opd_kl_min <= args.offline_direct_opd_kl_coef <= args.offline_direct_opd_kl_max:
                raise ValueError("initial Offline Direct-OPD KL coefficient must lie within adaptive bounds")
            if args.save is None or args.save_interval is None:
                raise ValueError("adaptive Offline Direct-OPD requires --save and --save-interval for controller state")
        if args.kl_coef != 0 or args.use_kl_loss or args.kl_loss_coef != 0:
            raise ValueError(
                "Offline Direct-OPD reads the initial-student reference from data; "
                "do not enable live --kl-coef/--use-kl-loss/--kl-loss-coef"
            )
        if args.offline_direct_opd_manifest is None:
            raise ValueError("--offline-direct-opd requires --offline-direct-opd-manifest")
        from slime.rollout.offline_direct_opd import validate_sealed_manifest

        manifest = validate_sealed_manifest(
            args.offline_direct_opd_manifest,
            expected_top_k=args.direct_opd_top_k,
        )
        compatibility = manifest.get("token_id_compatibility", {})
        if compatibility.get("mode") == "byte_level_exact_token_projection_v1":
            normalization_vocab_sizes = compatibility.get(
                "normalization_vocab_sizes",
                {},
            )
            action_vocab_size = normalization_vocab_sizes.get(
                "student",
                manifest["model_vocab_size"],
            )
        else:
            action_vocab_size = compatibility.get(
                "normalization_vocab_size",
                compatibility.get(
                    "model_vocab_size",
                    manifest["model_vocab_size"],
                ),
        ...
        if not 0 < action_vocab_size <= model_vocab_size:
            raise ValueError(
                "Offline Direct-OPD normalized student action vocabulary "
                f"{action_vocab_size} is outside model vocabulary "
                f"[1,{model_vocab_size}]"
            )
        args.offline_direct_opd_action_vocab_size = action_vocab_size
        if args.num_rollout is None:
            raise ValueError("--offline-direct-opd requires an explicit --num-rollout")
        required_rows = args.num_rollout * args.rollout_batch_size
        if manifest["total_rows"] < required_rows:
            raise ValueError(
                f"Offline Direct-OPD dataset has {manifest['total_rows']} rows, "
                f"but this run requires at least {required_rows} "
                "(--num-rollout * --rollout-batch-size)"
            )
        manifest_student_path = manifest["student_model"].get("path")
        if manifest_student_path is None:
            raise ValueError("Offline Direct-OPD manifest student_model is missing its locked path")
        student_mismatch = Path(args.hf_checkpoint).resolve() != Path(manifest_student_path).resolve()
        if args.offline_direct_opd_allow_student_mismatch and args.offline_direct_opd_loss_mode != "tilted_target":
            raise ValueError(
                "--offline-direct-opd-allow-student-mismatch is supported only with tilted_target"
            )
        if student_mismatch and not args.offline_direct_opd_allow_student_mismatch:
            raise ValueError(
                f"--hf-checkpoint {Path(args.hf_checkpoint).resolve()} does not match sealed student "
                f"{Path(manifest_student_path).resolve()}"
            )
        ...
        expected_generation_config = {
            ...
            "max_prompt_length": args.rollout_max_prompt_len,
            "responses_per_prompt": args.offline_direct_opd_responses_per_prompt,
            "top_k": args.direct_opd_top_k,
        }
        ...
        if mismatched_generation:
            raise ValueError(
                "Offline Direct-OPD generation config differs from training profile: "
                f"{mismatched_generation}"
            )
        if not isinstance(args.prompt_data, list) or len(args.prompt_data) != 1:
            raise ValueError("--offline-direct-opd requires exactly one --prompt-data directory")
        prompt_data_path = Path(args.prompt_data[0]).resolve()
        manifest_parent = Path(args.offline_direct_opd_manifest).resolve().parent
        if prompt_data_path != manifest_parent:
            raise ValueError(
                f"Offline Direct-OPD prompt data {prompt_data_path} must be the sealed manifest "
                f"directory {manifest_parent}"
            )
