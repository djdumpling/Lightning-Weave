...
def pack_sequences(
    ...
    candidate_ids: list[list[list[int]]] | None = None,
    behavior_topk_log_probs: list[list[list[float]]] | None = None,
    post_teacher_log_probs: list[list[list[float]]] | None = None,
    pre_teacher_log_probs: list[list[list[float]]] | None = None,
    student_ref_sampled_log_probs: list[list[float]] | None = None,
    ...
    for indices in partitions:
        ...
        if teacher_log_probs is not None:
            packed_batch["teacher_log_probs"] = torch.tensor(
                flat_teacher_log_probs,
                dtype=torch.float32,
                device=torch.cuda.current_device(),
            )
        # Offline Direct-OPD fields retain their per-sequence boundaries.  The
        # response lengths vary, while the candidate axis K is fixed by the
        # sealed manifest, so keeping a list avoids padding large [T,K]
        # tensors and lets ``unpack_sequences`` return the original rows.
        direct_opd_fields = {
            "candidate_ids": (candidate_ids, torch.long),
            "behavior_topk_log_probs": (behavior_topk_log_probs, torch.float32),
            "post_teacher_log_probs": (post_teacher_log_probs, torch.float32),
            "pre_teacher_log_probs": (pre_teacher_log_probs, torch.float32),
            "student_ref_sampled_log_probs": (
                student_ref_sampled_log_probs,
                torch.float32,
            ),
        }
        for key, (values, dtype) in direct_opd_fields.items():
            if values is not None:
                packed_batch[key] = [
                    torch.as_tensor(values[i], dtype=dtype) for i in indices
                ]
            ...
            if key not in instance:
                ...
                # For tensor attributes, we need to slice them appropriately
                    ...
                    if key in ["log_probs", "ref_log_probs", "cur_log_probs", "entropy"]:
                        ...
                        # Per-response values are packed separately from full-sequence tensors.
