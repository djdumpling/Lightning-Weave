...
def _rollout_log_preview(sample: Sample, *, max_chars: int = 2048) -> str:
    """Keep long offline code solutions from flooding Ray driver logs."""

    text = str(sample.prompt) + sample.response
    if len(text) <= max_chars:
        return text
    suffix_chars = min(256, max_chars // 4)
    prefix_chars = max_chars - suffix_chars
    omitted = len(text) - max_chars
    return f"{text[:prefix_chars]}... <{omitted} chars omitted> ...{text[-suffix_chars:]}"
...
def _is_lightning_opd(sample: Sample) -> bool:
    ...
    return (
        metadata.get("is_lightning_opd", False)
        or metadata.get("is_offline_opd", False)
        or metadata.get("is_offline_direct_opd", False)
    )
...
def _handle_lightning_opd_sample(sample: Sample, state: "GenerateState") -> Sample:
    ...
    if metadata.get("is_offline_direct_opd", False):
        from slime.rollout.offline_direct_opd import hydrate_offline_direct_opd_sample

        return hydrate_offline_direct_opd_sample(
            sample,
            state.tokenizer,
            expected_top_k=getattr(state.args, "direct_opd_top_k", None),
            # Argument validation has already verified every shard digest in
            # the sealed manifest before the RolloutManager is created.
            trusted_sealed=True,
        )
