...
def _validate_flashqla_runtime():
    if _parse_version(torch.__version__) < (2, 8):
        raise RuntimeError(
            f"FlashQLA backend requires PyTorch 2.8 or newer, got PyTorch {torch.__version__}."
        )
    ...
    if (major, minor) < (9, 0):
        raise RuntimeError(
            f"FlashQLA backend requires NVIDIA SM90 or newer, got sm{major}{minor}."
        )

    cuda_version = torch.version.cuda
    if cuda_version is not None and _parse_version(cuda_version) < (12, 8):
        raise RuntimeError(
            f"FlashQLA backend requires CUDA 12.8 or newer, got CUDA {cuda_version}."
        )
...
def get_chunk_gated_delta_rule(backend: str):
    if backend == "fla":
        try:
            ...
            raise ImportError(
                "Qwen GDN backend 'fla' requires flash-linear-attention."
            ) from exc
