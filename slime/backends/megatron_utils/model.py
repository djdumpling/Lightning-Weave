...
from .checkpoint import _is_megatron_checkpoint, load_checkpoint, save_checkpoint
...
def _diagnostic_rank_matches() -> bool:
    ...
    if not torch.distributed.is_initialized():
        return False
    requested_rank = int(os.environ.get("SLIME_DIAG_NONFINITE_GRADS_RANK", "-1"))
    return requested_rank < 0 or torch.distributed.get_rank() == requested_rank


def _diagnostic_tensor_values(value, limit: int = 64):
    if value is None:
        return None
    if isinstance(value, torch.Tensor):
        flattened = value.detach().reshape(-1)
        return flattened[:limit].cpu().tolist()
    if isinstance(value, (list, tuple)):
        return list(value[:limit])
    return repr(value)


def _log_nonfinite_main_grads(model) -> None:
    ...
    for chunk_index, model_chunk in enumerate(model):
        for name, parameter in model_chunk.named_parameters():
            ...
            if grad is None:
                continue
            named_grads.append((chunk_index, name, grad))
            finite_flags.append(torch.isfinite(grad).all())

    if not finite_flags:
        logger.error(
            "SLIME_DIAG_NONFINITE_GRADS rank=%s found no materialized gradients",
            torch.distributed.get_rank(),
        )
        return

    flags = torch.stack(finite_flags).cpu().tolist()
    bad = [entry for entry, is_finite in zip(named_grads, flags, strict=True) if not is_finite]
    if not bad:
        logger.info(
            "SLIME_DIAG_NONFINITE_GRADS rank=%s all %s parameter gradients finite",
            torch.distributed.get_rank(),
            len(named_grads),
        )
        return

    for chunk_index, name, grad in bad:
        ...
        logger.error(
            "SLIME_DIAG_NONFINITE_GRAD rank=%s chunk=%s name=%s shape=%s "
            "dtype=%s nan=%s posinf=%s neginf=%s finite_abs_max=%s",
            torch.distributed.get_rank(),
            chunk_index,
            name,
            tuple(grad.shape),
            ...
            int(torch.isposinf(grad).sum().item()),
            int(torch.isneginf(grad).sum().item()),
            abs_max,
        )


def _log_diagnostic_model_parameters(model, stage: str) -> None:
    if not _diagnostic_rank_matches():
        return
    ...
    for chunk_index, model_chunk in enumerate(model):
        for name, parameter in model_chunk.named_parameters():
            ...
            if not bool(finite.all().item()):
                ...
                logger.error(
                    "SLIME_DIAG_NONFINITE_PARAM rank=%s stage=%s chunk=%s "
                    "name=%s shape=%s dtype=%s nan=%s posinf=%s neginf=%s "
                    "finite_abs_max=%s",
                    rank,
                    stage,
                    chunk_index,
                    ...
                    tuple(data.shape),
                    data.dtype,
                    int(torch.isnan(data).sum().item()),
                    int(torch.isposinf(data).sum().item()),
                    int(torch.isneginf(data).sum().item()),
                    finite_abs_max,
                )
            if name.endswith((".A_log", ".dt_bias")):
                logger.error(
                    "SLIME_DIAG_GDN_HEAD_PARAM rank=%s stage=%s chunk=%s "
                    "name=%s values=%s",
                    rank,
                    stage,
                    chunk_index,
                    name,
                    data.float().cpu().tolist(),
                )
...
def forward_only(
    ...
    Args:
        ...
        extra_batch_keys: Extra rollout fields passed to ``f`` as ``batch``.
...
def train_one_step(
    ...
    _log_diagnostic_model_parameters(
        model,
        f"rollout_{rollout_id}_step_{step_id}_before_forward",
    )
    ...
    def forward_step(data_iterator: DataIterator, model: GPTModel, return_schedule_plan: bool = False) -> tuple[
        ...
        batch = get_batch(
            ...
            [
                ...
                "candidate_ids",  # Offline Direct-OPD [response_length, K]
        ...
        if _diagnostic_rank_matches():
            packed_seq_params = batch["packed_seq_params"]
            logger.error(
                "SLIME_DIAG_BATCH rank=%s rollout=%s step=%s tokens_shape=%s "
                "total_lengths=%s response_lengths=%s cu_seqlens_q=%s",
                torch.distributed.get_rank(),
                rollout_id,
                step_id,
                tuple(batch["tokens"].shape),
                _diagnostic_tensor_values(batch["total_lengths"]),
                _diagnostic_tensor_values(batch["response_lengths"]),
                _diagnostic_tensor_values(
    ...
    if valid_step:
        _log_diagnostic_model_parameters(
            model,
            f"rollout_{rollout_id}_step_{step_id}_before_optimizer",
        )
        ...
        _log_diagnostic_model_parameters(
            model,
            f"rollout_{rollout_id}_step_{step_id}_after_optimizer",
        )
...
def finalize_model_grads_with_empty_cache(*args, **kwargs):
    ...
    if model is not None:
        _log_nonfinite_main_grads(model)
...
def initialize_model_and_optimizer(
    ...
    is_megatron_checkpoint = _is_megatron_checkpoint(args.load)
    initialization_checkpoint = is_initialization_checkpoint(
        args.load,
        is_megatron_checkpoint=is_megatron_checkpoint,
    )
    expected_scheduler_steps = (iteration + 1) * args.rollout_batch_size * args.n_samples_per_prompt
    full_resume = (
        is_megatron_checkpoint
        and not initialization_checkpoint
        and not args.no_load_optim
        and not args.finetune
    )

    # Megatron restores opt_param_scheduler (including num_steps) together
    # with the optimizer on a full resume. Advancing it again here would
    # double-count all completed updates.
    if full_resume:
        reapply_runtime_lr_bounds(
            args,
            optimizer,
            opt_param_scheduler,
            expected_num_steps=expected_scheduler_steps,
        )
        if opt_param_scheduler.num_steps != expected_scheduler_steps:
            raise RuntimeError(
                "Restored scheduler is not aligned with the checkpoint: "
                f"actual num_steps={opt_param_scheduler.num_steps}, "
                f"expected={expected_scheduler_steps}, iteration={iteration}"
            )
    elif (
        is_megatron_checkpoint
        and not initialization_checkpoint
        ...
        opt_param_scheduler.step(increment=expected_scheduler_steps)

    loaded_rollout_id = -1 if initialization_checkpoint else iteration
    return model, optimizer, opt_param_scheduler, loaded_rollout_id
